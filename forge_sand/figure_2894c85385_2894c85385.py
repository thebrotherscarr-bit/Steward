...
# ---------------------------------------------------------------------------
# THE GATE — the board watches the pack. The waterfall: Manjuel watches,
# Steward does; Steward watches, the Forge does; the Forge watches, the board
# does; the BOARD watches, the pack does; the pack watches, the sand does.
# The farther from Manjuel the more movement, but less authority — at this
# doorway the board holds the authority and the pack holds only the motion.
#
# Clearance is checked against REALITY — the rack as it is, the sand as it
# is — never against the order's own claims. Every clearance, granted or
# refused, is witnessed to the ledger. Refused means NOTHING moves: the
# doorway is never unsecured.
# ---------------------------------------------------------------------------
SAND = os.path.join(HERE, "forge_sand")
MAX_MINUTES = 60      # no allotment exceeds the hour — the garden is for movement, not residence


def _clay_exists(name):
    """Is this little real clay in the sand? Exact file or prefix match — the
    same eyes the pack uses to find it, used EARLIER, at the door. A ghost is
    caught here, not discovered mid-mission."""
    if not name:
        return False
    if os.path.exists(os.path.join(SAND, name + ".py")):
        return True
    if os.path.isdir(SAND):
        for f in os.listdir(SAND):
            if f.startswith(name) and f.endswith(".py"):
                return True
    return False


def clear_mission(order):
    """The board's clearance — the gate the pack must ask before motion.
    Five checks against reality; one witnessed verdict. Advice elsewhere is
    advice; THIS is the one place the board rules, because a watcher who
    cannot refuse is not a watcher."""
    checks, refusals = {}, []

    scope_ok = bool(str(order.get("scope", "")).strip())
    checks["scope_present"] = scope_ok
    if not scope_ok:
        refusals.append("no scope given — a mission without a wall is not a mission")

    littles = [str(l).strip() for l in order.get("littles", [])]
    checks["littles_named"] = bool(littles)
    if not littles:
        refusals.append("no littles named — nothing to move is nothing to do")

    ghosts = [l for l in littles if not _clay_exists(l)]
    checks["littles_are_real_clay"] = not ghosts
    if ghosts:
        refusals.append("no such clay in the sand: %s" % ", ".join(ghosts))

    rack = _load_rack()
    missing_tools = [t for t in order.get("tools", []) if t not in rack]
    checks["tools_on_rack"] = not missing_tools
    if missing_tools:
        refusals.append("tools not on the rack: %s" % ", ".join(missing_tools))

    try:
        minutes = float(order.get("minutes", 0))
    except Exception:
        minutes = 0
    allot_ok = 0 < minutes <= MAX_MINUTES
    checks["allotment_sane"] = allot_ok
    if not allot_ok:
        refusals.append("allotment must be within (0, %d] minutes, got %r"
                        % (MAX_MINUTES, order.get("minutes")))

    cleared = not refusals
    _mark("gate", {"ticket": order.get("ticket"), "verdict": "CLEARED" if cleared else "REFUSED",
                   "checks": checks, "refusals": refusals})
    return {"cleared": cleared, "verdict": "CLEARED" if cleared else "REFUSED",
            "checks": checks, "refusals": refusals,
            "note": "checked against the rack and the sand as they ARE, not as the order claims"}